...
(function(root, factory) {
  ...
  var uid = ['0', '0', '0']; // define uid number
  ...
  _.extend(State.prototype, {
    ...
    entry : function(stateContext) {
      _.forEach(this.entryActions, function(action) {
        stateContext.executionService.defer(action, null, this.getStateId(),
          stateContext.event, stateContext.context, stateContext.stateMachine);
      }.bind(this));
    },
    ...
    exit : function(stateContext) {
      ...
      _.forEach(this.exitActions, function(action) {
        stateContext.executionService.defer(action, this.getStateId(), null,
          stateContext.event, stateContext.context, stateContext.stateMachine);
      }.bind(this));
    },
    ...
    _beforeFinalized : function() {
      this._check();
      // update historical state for hierarchical state
      if(this.parentState && this.parentState.getHistoryType()!==HistoryType.NONE) {
        var lastExitAction = {};
        lastExitAction[Keywords.FUNCTION] = _.partial(
          function(stateId, parentStateId) {
            this._setLastActiveChildStateFor(parentStateId, stateId);
          }, this.stateId, this.parentState.stateId
        );
        lastExitAction[Keywords.WEIGHT] = Weight.MIN_WEIGHT;
        lastExitAction[Keywords.FUNC_NAME] = "__updateHistory";
        this.addStateAction(lastExitAction, false);
      }
    },

    finalized : function() {
      this._beforeFinalized();
  ...
  var Transition = function(sourceState, targetState, event, condition, priority, transitionType) {
    ...
    this.event = event;
    this.actions = [];
    this.condition = _.isFunction(condition) ? condition : _.isString(condition) ?
      function(context) {
        return this.options.exprEvalService(condition, context); // this->statemachine
      } : Conditions.Always;
    this.priority = priority || Priority.NORMAL;
    this.transitionType = _.isUndefined(transitionType) ? TransitionType.EXTERNAL : transitionType.valueOf();
    this.isFinalized = false;
  };

  _.extend(Transition.prototype, {
    internalFire : function(stateContext) {
      if(this.condition.call(stateContext.stateMachine, stateContext.context)) {
    ...
    _transit : function(stateContext) {
      _.forEach(this.actions, function(action) {
        stateContext.executionService.defer(action, this.sourceState.getStateId(), this.targetState.getStateId(),
          stateContext.event, stateContext.context, stateContext.stateMachine);
      }.bind(this));
    },
  ...
  Events.BEFORE_ACTION_EXECUTE = "beforeActionExecute";
  Events.AFTER_ACTION_EXECUTE = "afterActionExecute";

  // action execution service class
  var ExecutionService = function() {
    this.numOfActions = 0;
    this.actions = [];
  };

  _.extend(ExecutionService.prototype, {
    defer : function(action, from, to, event, context, stateMachine) {
      var position = ++this.numOfActions;
      var isDebugInfoEnabled = stateMachine.options.isDebugInfoEnabled;
      this.actions.push({
        invoke : function() {
          var totalActionNum = stateMachine.executionService.numOfActions;
          if(isDebugInfoEnabled) {
            console.log("Invoke action '"+action.name+"' ("+position+" of "+totalActionNum+") begin.");
          }
          stateMachine.trigger(Events.BEFORE_ACTION_EXECUTE, action.name, from, to, event, context, [position, totalActionNum]);
          action.func.call(stateMachine/*->this*/, from, to, event, context);
          stateMachine.trigger(Events.AFTER_ACTION_EXECUTE, action.name, from, to, event, context, [position, totalActionNum]);
        }
      });
    },
  ...
  var Options = Squirrel.Options = {
    ...
    exprEvalService : function(expr) {
      var expression = "";
      if(arguments.length>1) {
        expression+="var context=arguments[1]";
      }
      if(arguments.length>2) {
        expression+=";machine=arguments[2]";
      }
      expression+=";"+expr;
  ...
  var StateMachine = Squirrel.StateMachine = function(initialState, options) {
    ...
    if(!this.initialState) {
      initialRawState = _.find(this.constructor._states, function(s) { return s.isInitialState(); });
  ...
  _.extend(StateMachine.prototype, Events, {
    ...
    callsuper : function(funcName) {
      var superFunc = this.constructor.__super__[funcName];
      if(!superFunc) {
        throw new Error("cannot find function '"+funcName+"' in super class '"+this.getClassName()+"'.");
      }
      return superFunc.apply(this, _.rest(arguments));
    },
    ...
    start : function(context) {
      ...
      var initialRawState = this._rawState(this.initialState),
      stateContext = {
        from : null,
    ...
    _currentRawState : function() {
      return this.constructor._states[this.currentState];
    },

    _rawState : function(stateId) {
      return this.constructor._states[stateId];
    },
    ...
    transitionBegin : function(fromStateId, event, context) {
      if(this.options.isDebugInfoEnabled) {
        console.log("StateMachine["+this.getId()+"]: Transition from '"+fromStateId+"' on event '"+event+
    ...
    transitionCompleted : function(fromStateId, currentStateId, event, context) {
      if(this.options.isDebugInfoEnabled) {
        console.log("StateMachine["+this.getId()+"]: Transition from '"+fromStateId+"' to '"+currentStateId+
    ...
    methodMissing : function(methodName, fromStateId, toStateId, event, context, stateMachine) {
      if(this.options.isDebugInfoEnabled) {
        console.log("StateMachine["+this.getId()+"]: Cannot found method '"+methodName+"' from '"+fromStateId+"' to '"+
          toStateId+"' on '"+event+"' with context '"+context+"'.");
        console.log("StateMachine["+this.getId()+"]: \n"+stateMachine.getEffectiveDefinition());
      }
    }
  });
  ...
  var extend = function(stateMachineProps, staticProps) {
    ...
    function buildActions(actionDesc, stateMachinePrototype) {
      ...
      var func = null, weight = Weight.NORMAL_WEIGHT, name, actions=[];
      if(_.isFunction(actionDesc)) {
        ...
        name = values[0];
        func = stateMachinePrototype[name] ||
          _.partial(stateMachinePrototype.methodMissing, name);
        ...
        name = actionDesc[Keywords.FUNC_NAME];
      ...
      if(actions.length === 0) {
        actions.push({func: func, name: name, weight: weight});
      }
      return actions;
    }
    ...
    var stateMachineDefinition = _.isArray(stateMachineProps) ? (stateMachineProps[1] || {}) : stateMachineProps,
      stateMachineClass = classExtend.call(this, stateMachineDefinition, staticProps),
      ...
      states = stateMachineClass._states = {};

    stateMachineClass.classname = _.isArray(stateMachineProps) ? stateMachineProps[0] :
      (stateMachineProps[Keywords.CLASSNAME] || "unknown_type");

    if(Options.isDebugInfoEnabled) {
      console.log(
