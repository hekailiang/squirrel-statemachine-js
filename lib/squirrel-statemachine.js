...
(function(root, factory) {
  ...
  _.extend(State.prototype, {
    ...
    exit : function(stateContext) {
      ...
      if(this.parentState && this.parentState.getHistoryType()!==HistoryType.NONE) {
        stateContext.stateMachine.setLastActiveChildStateFor(this.parentState.stateId, this.stateId);
      }
    },
  ...
  var StateMachine = Squirrel.StateMachine = function(initialState, options) {
    var initialRawState;
    this.initialState = initialState || this[Keywords.DEFINITION].initial;
  ...
  _.extend(StateMachine.prototype, Events, {
    ...
    unfoundMethod : function(methodName, fromStateId, toStateId, event, context, stateMachine) {
      console.log("Cannot found method '"+methodName+"' from '"+fromStateId+"' to '"+
        toStateId+"' on '"+event+"' with context '"+context+"'.");
      console.log("State Machine Definition: \n"+stateMachine.getStringifyDefinition());
    },

    getEffectiveDefinition : function() {
      return StateMachine.getDefinition(this);
    },

    getStringifyDefinition : function() {
      return JSON.stringify(this.getEffectiveDefinition(), null, 2);
    }
  });
  ...
  var buildActions = function(actionDesc, stateMachinePrototype) {
    ...
    if(_.isFunction(actionDesc)) {
      ...
      func = stateMachinePrototype[values[0]] ||
        _.partial(stateMachinePrototype.unfoundMethod, values[0]);
  ...
  var _mergeDefinition = function(target, source) {
        ...
        _.forEach([Keywords.ON_ENTRY, Keywords.ON_EXIT], function(keyword) {
          ...
          if(_.has(sourceState, keyword)) {
            if(_.has(targetState, keyword)) {
              targetState[keyword] = [].concat(targetState[keyword], sourceState[keyword]);
            } else {
    ...
    defaultDeep(target[Keywords.TRANSITIONS], source[Keywords.TRANSITIONS]);

    return target;
  };

  var computeDefinition = function(stateMachine) {
    ...
    while(currentStateMachineProto) {
      if(_.has(currentStateMachineProto, Keywords.DEFINITION)) {
        _mergeDefinition(mergedDefinition, currentStateMachineProto[Keywords.DEFINITION]);
      }
  ...
  var buildStateModel = function(stateInfo, stateId, states, stateMachinePrototype) {
    ...
    if(_.has(stateInfo, Keywords.CHILD_STATES) && _.isPlainObject(stateInfo[Keywords.CHILD_STATES])) {
      _.forOwn(stateInfo[Keywords.CHILD_STATES], function(childStateInfo, childStateId) {
