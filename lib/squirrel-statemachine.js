...
(function(root, factory) {
  ...
  _.extend(State.prototype, {
    ...
    clone : function(states) {
      var i, len, key, cloned = getState(this.stateId, states);
      cloned.parent = this.parent;
      cloned.historyType = this.historyType;
      cloned.compositionType = this.compositionType;
      cloned.level = this.level;
      cloned.isFinal = this.isFinal;
      // clone actions
      for(i=0, len=this.entryActions.length; i<len; ++i) {
        cloned.entryActions.push(_.cloneDeep(this.entryActions[i]));
      }
      for(i=0, len=this.exitActions.length; i<len; ++i) {
        cloned.exitActions.push(_.cloneDeep(this.exitActions[i]));
      }
      // clone transitions
      for(key in this.transitionMap) {
        if(this.transitionMap.hasOwnProperty(key)) {
          cloned.transitionMap[key] = [];
          for(i=0, len=this.transitionMap[key].length; i<len; ++i) {
            cloned.transitionMap[key].push(this.transitionMap[key][i].clone(states));
          }
        }
      }
      // clone child states
      for(i=0, len=this.childStates.length; i<len; ++i) {
        cloned.childStates.push(this.childStates[i].clone());
      }
      return cloned;
    }
  ...
  _.extend(Transition.prototype, {
    ...
    clone : function(states) {
      var i, len, cloned = new Transition(
        getState(this.sourceState.stateId, states), 
        getState(this.targetState.stateId, states), 
        this.event, this.condition, this.priority, this.transitionType);
      for(i=0, len=this.actions.length; i<len; ++i) {
        cloned.actions.push(_.cloneDeep(this.actions[i]));
      }
      return cloned;
    }
  ...
  var extend = function(stateMachineProps, staticProps) {
    var stateMachineClass = classExtend.call(this, stateMachineProps, staticProps),
      ...
      hasOwnDefinition = stateMachinePrototype.hasOwnProperty("definition"),
      hasOwnStates = hasOwnDefinition && !!stateMachinePrototype.definition.states,
      stateInfoList = hasOwnStates ? stateMachinePrototype.definition.states : {},
      hasOwnTransitions = hasOwnDefinition && !!stateMachinePrototype.definition.transitions,
      transitionInfoList = hasOwnTransitions ? stateMachinePrototype.definition.transitions : [],
      stateId, stateModel, stateInfo, action, 
      transitionInfo, fromState, toState, transition, 
      states = stateMachinePrototype._states = {}, i, key;

    // clones parent states
    for(key in stateMachineClass.__super__._states) {
      if(stateMachineClass.__super__._states.hasOwnProperty(key)) {
        stateMachineClass.__super__._states[key].clone(states);
      }
    }
    ...
    for(stateId in stateInfoList) {
      if (stateInfoList.hasOwnProperty(stateId)) {
        ...
        if(stateInfo.hasOwnProperty("onEntry")) {
          action = buildAction(stateInfo.onEntry, stateMachinePrototype);
          stateModel.addStateAction(action, true);
        }
        if(stateInfo.hasOwnProperty("onExit")) {
          action = buildAction(stateInfo.onExit, stateMachinePrototype);
    ...
    for(i=0; i<transitionInfoList.length; ++i) {
      ...
      action = buildAction(transitionInfo.perform, stateMachinePrototype);
      if(!!action) {
