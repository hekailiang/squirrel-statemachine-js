...
(function(root, factory) {
  ...
  _.extend(StateMachine.prototype, Events, {
    ...
    getEffectiveDefinition : function() {
      return JSON.stringify(StateMachine.getDefinition(this), null, 2);
    }
  });
  ...
  var _mergeDefinition = function(target, source) {
    ...
    _.forOwn(source.states, function(sourceState, key) {
      ...
      if(_.has(target.states, key)) {
        ...
        _.forEach([Keywords.ON_ENTRY, Keywords.ON_EXIT], function(keyword) {
          var targetState = target.states[key];
          if(_.has(sourceState, keyword)) {
            if(_.has(target.states[key], keyword)) {
        ...
        target.states[key] = _.cloneDeep(sourceState);
      }
    });
  ...
  var buildStateModel = function(stateInfo, stateId, states, stateMachinePrototype) {
    ...
    if(_.has(stateInfo, Keywords.FINAL)) {
      parentStateModel.setFinalSate(!!stateInfo.final);
    }
    if(_.has(stateInfo, Keywords.INITIAL)) {
      parentStateModel.setInitialState(!!stateInfo.initial);
    }
    if(_.has(stateInfo, Keywords.HISTORY_TYPE)) {
      parentStateModel.setHistoryType(stateInfo.historyType);
    }
    if(_.has(stateInfo, Keywords.CHILD_STATES) && _.isPlainObject(stateInfo.childStates)) {
      _.forOwn(stateInfo.childStates, function(childStateInfo, childStateId) {
